...
import { InvoiceFormValuesType } from "./InvoiceFormValuesType";
...
interface InputFieldProps {
  label: string;
  name: keyof InvoiceFormValuesType; // Use keyof to specify keys from CustomerFormValuesType
  boxcolor: string;
  type: "text" | "password" | "email" | "button" | "submit" | "reset" | undefined;
  placeholder: string;
  handleChange: ChangeEventHandler<HTMLInputElement>;
  values: InvoiceFormValuesType; // Change the type here
  icon: keyof typeof MuiIcons;
}

  

function InputFieldInvoice({ label, name, type, placeholder, handleChange, values, boxcolor, icon }: InputFieldProps) {
...
export default InputFieldInvoice;
