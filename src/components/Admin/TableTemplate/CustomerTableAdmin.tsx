...
const CustomerTableAdmin = ({ tablehead }: TableProps) => {
  ...
  const downloadDocs = async (phone:String,firstName:String,lastName:String) => {
    ...
    try {
        // Make an HTTP GET request to retrieve customer data
        const response = await fetch(`http://localhost:5001/api/customers/findbyPhone/${phone}`, {
            method: 'GET',
            headers: headers
        });
        ...
        if (response.ok) {
            ...
            const nic = data[0].nicDoc[0].split('\\').pop();
            const br = data[0].brDoc[0].split('\\').pop();
            const other = data[0].otherDoc[0].split('\\').pop();

            // Create a new JSZip instance
            const zip = new JSZip();

            // Add files to the zip
            const nicData = await axios.get(`http://localhost:5001/api/customers/download/${nic}`, { responseType: 'blob' });
            zip.file('NIC.jpg', nicData.data);

            const brData = await axios.get(`http://localhost:5001/api/customers/download/${br}`, { responseType: 'blob' });
            zip.file('BR.jpg', brData.data);

            const otherData = await axios.get(`http://localhost:5001/api/customers/download/${other}`, { responseType: 'blob' });
            zip.file('Other.jpg', otherData.data);

            // Generate the zip file asynchronously
            const zipBlob = await zip.generateAsync({ type: 'blob' });

            // Trigger download of the zip file
            saveAs(zipBlob, `${firstName} ${lastName} - CustomerDocs.zip`);
                ...
                <tr key={index}>
                  ...
                  <td className="border-b border-[#eee] py-5 px-4 pl-9 dark:border-strokedark xl:pl-11 ">
                    ...
                    <FontAwesomeIcon
                      ...
                      onClick={() => downloadDocs(customerDataItem.phone,customerDataItem.firstname,customerDataItem.lastname)}/>
                    </h5>
                    <button 
